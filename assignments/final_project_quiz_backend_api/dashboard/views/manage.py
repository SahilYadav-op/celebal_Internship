...
def show():
    st.header("Manage Questions & Choices")
    ...
    info_cols[2].metric("Status", "Ready" if len(choices) >= 2 and n_correct >= 1 else "Incomplete")

    # edit or delete the selected question
    edit_col, del_col = st.columns([4, 1])
    with edit_col:
        with st.expander("Edit question text / category"):
    ...
    with del_col:
        st.write("")
        st.write("")
        if st.button("Delete", key=f"del_q_btn_{q['id']}", type="secondary", use_container_width=True):
                    ...
                    if save_col.form_submit_button("Save", type="primary"):
                        if not new_ct.strip():
                            ...
                            if res:
                                st.success("Updated.")
                                st.rerun()
                    if del_col2.form_submit_button("Delete this choice"):
